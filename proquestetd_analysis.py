...
def main():
    ...
    parser = ArgumentParser(usage='%(prog)s [options] data_directory')
    ...
    if os.path.isfile(args.datafile):
        for event, elem in etree.iterparse(args.datafile):
            if elem.tag == "DISS_submission":
                r = Record(elem, args)
                record_id = args.datafile.strip('.xml')
        ...
        for subdir, dirs, files in os.walk(args.datafile):
            for file in files:
                if file.endswith('DATA.xml'):
                    for event, elem in etree.iterparse(subdir + os.sep + file):
                        if elem.tag == "DISS_submission":
                            r = Record(elem, args)
                            record_id = file.strip('.xml')
